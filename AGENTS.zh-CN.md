...
Windows PowerShell 也可以使用容器基础设施与本地 Python 进程：
...
该脚本启动 V3 基础设施、本地 MCP、API 和诊断 Worker。选定的本地 Python 环境必须已安装项目
依赖。根目录 Windows `run.ps1` 仍是本地兼容入口，不会启动完整 V3 Postgres/Worker 拓扑；
完整容器化环境应使用 Compose 的 `app` Profile。PowerShell 停止脚本默认保留 Docker 数据卷；
传入 `-Infra` 才会额外执行 `docker compose down`。
