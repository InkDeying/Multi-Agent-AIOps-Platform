...
function Start-OpenWebSearchDaemon {
    ...
    $baseUrl = Get-EnvValue -Name "OPEN_WEBSEARCH_BASE_URL" -DefaultValue "http://127.0.0.1:3310"
    $port = Get-PortFromUrl -Url $baseUrl -DefaultPort 3310
    if (Test-TcpPort -HostName "127.0.0.1" -Port $port) {
...
if ($Stop) {
    ...
    $openWebSearchStopPort = Get-PortFromUrl -Url (Get-EnvValue -Name "OPEN_WEBSEARCH_BASE_URL" -DefaultValue "http://127.0.0.1:3310") -DefaultPort 3310
    9105,9106,9108,9109,9111,9900,$openWebSearchStopPort | ForEach-Object {
...
if (-not $NoMcp) {
    Start-PythonServer -Name "system_server" -Script "$ProjectRoot\mcp_servers\system_server.py" -Port 9105
    Start-PythonServer -Name "websearch_server" -Script "$ProjectRoot\mcp_servers\websearch_server.py" -Port 9106
    Start-PythonServer -Name "winlog_server" -Script "$ProjectRoot\mcp_servers\winlog_server.py" -Port 9108
    Start-PythonServer -Name "network_server" -Script "$ProjectRoot\mcp_servers\network_server.py" -Port 9109
    Start-PythonServer -Name "docker_server" -Script "$ProjectRoot\mcp_servers\docker_server.py" -Port 9111
} else {
