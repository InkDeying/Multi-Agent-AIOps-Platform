...
Local Windows PowerShell stack with containerized infrastructure and local Python
processes:
...
This starts the V3 infrastructure, local MCP services, API, and diagnosis
workers. It requires the selected local Python environment to have the project
dependencies installed. The root `run.ps1` launcher remains a compatibility
entry point and does not start the complete V3 Postgres/worker topology. Use the
Compose `app` profile for a fully containerized stack. The PowerShell stop script
keeps Docker volumes by default; pass `-Infra` to also run `docker compose down`.
