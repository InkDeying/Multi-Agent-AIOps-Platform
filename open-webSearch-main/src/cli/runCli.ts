...
function getDefaultDaemonBaseUrl(): string {
    return process.env.OPEN_WEBSEARCH_DAEMON_URL || `http://127.0.0.1:${process.env.OPEN_WEBSEARCH_DAEMON_PORT || '3310'}`;
}
...
export function parseStatusArgs(argv: string[]): ParsedStatusArgs {
    let baseUrl = process.env.OPEN_WEBSEARCH_DAEMON_URL || `http://127.0.0.1:${process.env.OPEN_WEBSEARCH_DAEMON_PORT || '3310'}`;
    let json = false;
...
export function parseServeArgs(argv: string[]): ParsedServeArgs {
    ...
    let port = Number(process.env.OPEN_WEBSEARCH_DAEMON_PORT || '3310');
