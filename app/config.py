...
class Settings(BaseSettings):
    ...
    deepseek_api_key: str = Field(
        default="", description="DeepSeek API Key (platform.deepseek.com)"
    )
    ...
    milvus_collection: str = Field(
        default="multi_agent_kb", description="Collection 名"
    )
    ...
    rag_chunk_size: int = Field(
        default=300, description="Child 块大小 (字符, embedding 用; 小块召回更准)"
    )
    rag_chunk_overlap: int = Field(default=50, description="Child 块间 overlap (字符)")
    rag_parent_max_chars: int = Field(
        default=2400, description="Parent 块字符上限; 超过则二次切"
    )
    ...
    database_pool_min_size: int = Field(
        default=1, description="Postgres 连接池最小连接数"
    )
    database_pool_max_size: int = Field(
        default=10, description="Postgres 连接池最大连接数"
    )
    ...
    rag_chat_memory_enabled: bool = Field(
        default=False, description="是否启用 RAG Chat Redis 会话记忆"
    )
    rag_chat_history_turns: int = Field(
        default=3, description="回答时注入最近 N 轮对话"
    )
    rag_chat_memory_ttl_sec: int = Field(
        default=604800, description="RAG Chat 会话记忆 TTL 秒数"
    )
    rag_chat_rewrite_enabled: bool = Field(
        default=True, description="是否启用多轮问题改写"
    )
    rag_chat_compact_enabled: bool = Field(
        default=True, description="是否启用长会话摘要压缩"
    )
    rag_chat_max_messages: int = Field(
        default=12, description="超过多少条消息触发 compact"
    )
    rag_chat_compact_keep_messages: int = Field(
        default=6, description="compact 后保留最近多少条原文消息"
    )
    rag_chat_summary_max_chars: int = Field(
        default=1200, description="会话摘要最大字符数"
    )
    ...
    rag_chat_web_search_enabled: bool = Field(
        default=False, description="是否允许 RAG Chat 使用受限联网搜索"
    )
    rag_chat_web_search_max_results: int = Field(
        default=3, description="RAG Chat 联网搜索最大结果数"
    )
    ...
    mcp_websearch_transport: str = Field(
        default="streamable-http", description="联网搜索 MCP 传输"
    )
    mcp_websearch_url: str = Field(
        default="http://localhost:9106/mcp", description="联网搜索 MCP URL"
    )
    mcp_winlog_transport: str = Field(
        default="streamable-http", description="Windows 事件日志 MCP 传输"
    )
    mcp_winlog_url: str = Field(
        default="http://localhost:9108/mcp", description="Windows 事件日志 MCP URL"
    )
    mcp_network_transport: str = Field(
        default="streamable-http", description="网络诊断 MCP 传输"
    )
    mcp_network_url: str = Field(
        default="http://localhost:9109/mcp", description="网络诊断 MCP URL"
    )
    mcp_docker_transport: str = Field(
        default="streamable-http", description="Docker 管理 MCP 传输"
    )
    mcp_docker_url: str = Field(
        default="http://localhost:9111/mcp", description="Docker 管理 MCP URL"
    )

    # ==================== Agent ====================
    agent_max_steps: int = Field(
        default=5, description="Plan-Execute 最大步骤 (防死循环)"
    )
    ...
    agent_max_concurrency: int = Field(
        default=2, description="AIOps Agent 最大并发诊断数"
    )
    ...
    harness_max_total_tokens: int = Field(
        default=0, description="单次 Harness run 的总 token 硬上限, 0 表示不限制"
    )
    harness_max_total_ms: int = Field(
        default=0, description="单次 Harness run 的总耗时硬上限, 0 表示不限制"
    )
    harness_budget_warn_ratio: float = Field(
        default=0.8, description="达到预算比例后输出 warning 事件"
    )
