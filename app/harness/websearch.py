...
调用方:
  ...
  - app.services.rag.web_context: 联网搜索结果构建
...
def get_open_websearch_config() -> dict[str, Any]:
    try:
        ...
        return {
            "base_url": (
                os.environ.get("OPEN_WEBSEARCH_BASE_URL") or "http://127.0.0.1:3310"
            ).strip(),
            "engine": (os.environ.get("OPEN_WEBSEARCH_ENGINE") or "bing").strip(),
            "search_mode": (
                os.environ.get("OPEN_WEBSEARCH_SEARCH_MODE") or "auto"
            ).strip(),
...
def search_open_websearch(query: str, max_results: int) -> List[Dict[str, Any]]:
    """open-webSearch local daemon provider. 默认地址: http://127.0.0.1:3310."""
    import httpx
...
def search(
    query: str, max_results: int, provider: str | None = None
) -> List[Dict[str, Any]]:
    ...
    raise ValueError(
        f"未知 web_search provider: {p!r} (可选 open_websearch / mock / ddgs)"
    )
