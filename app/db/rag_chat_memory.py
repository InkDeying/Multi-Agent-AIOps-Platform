"""RAG Chat 会话与诊断报告的 Redis 存储适配器.

会话消息、会话摘要和跨 session 的最近诊断报告共享同一个 Redis 客户端，
但使用不同的 key 域。
"""
...
async def append_diagnosis_report(
    report: str,
    *,
    session_id: str | None = None,
) -> None:
    """写入最近诊断报告，供 RAG Chat 的受限联网判断使用."""
    if not report:
        return
    client = await _get_redis()
    if client is None:
    ...
    try:
        await client.lpush(
            _DIAGNOSIS_REPORTS_KEY,
            json.dumps(payload, ensure_ascii=False),
        )
        await client.ltrim(
            _DIAGNOSIS_REPORTS_KEY,
            0,
            _DIAGNOSIS_REPORTS_MAX - 1,
        )
        await client.expire(
            _DIAGNOSIS_REPORTS_KEY,
            max(60, settings.rag_chat_memory_ttl_sec),
        )
    except Exception as exc:
        logger.warning(
            f"[chat-memory] 诊断报告写入失败: {type(exc).__name__}: {exc}"
        )
...
async def get_recent_diagnosis_reports(
    ...
    try:
        ...
        logger.warning(
            f"[chat-memory] 诊断报告读取失败: {type(exc).__name__}: {exc}"
        )
        return []
