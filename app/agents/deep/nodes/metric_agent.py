"""MetricAgent —— Deep Diagnosis 的指标专业 Agent。
...
Deep graph 契约:
  - 输入只读取 scoped 的 input/task 元信息，不读取其他 Agent 的中间推理;
  - 成功返回一条 metric_snapshot Evidence，失败返回带 error metadata 的占位 Evidence;
  - 内部 messages 不进入共享 DeepDiagnosisState。
"""
...
def _load_metric_tools() -> List[Any]:
    """延迟导入: 避免 deep graph 装配时就拉起 langchain @tool。
