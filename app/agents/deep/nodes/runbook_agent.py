"""RunbookAgent —— Deep Diagnosis 的 SOP/Runbook 检索专业 agent。
...
Deep graph 契约:
  - 只摘录 SOP、Runbook 和排查步骤，不做根因判定，不执行处置命令;
  - 与 LogAgent 共用知识库工具，但通过 scoped prompt 保持职责分离;
  - 成功返回一条 runbook_match Evidence，失败仍返回可归并的 error Evidence。
"""
