"""LogAgent —— Deep Diagnosis 的日志/知识检索专业 Agent。
...
Deep graph 契约:
  - 只接收当前故障现象和任务元信息，不共享 Metric/Infra Agent 的内部对话;
  - 成功返回一条 log_excerpt Evidence，失败返回可审计的 error Evidence;
  - 该 Agent 检索的是日志模板/告警规则/SOP，不代表已经连接真实日志后端。
"""
...
def _load_log_tools() -> List[Any]:
    """延迟导入: 避免 deep graph 装配时拉起 langchain @tool 子树。"""
