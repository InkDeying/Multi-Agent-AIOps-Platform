...
三者都从 ``app.agents.deep.specialists`` 的规格表派生，避免两处各写一份。
真实 Agent 的工具与 LLM 只在首次执行时导入，构图阶段不拉起完整依赖。
"""
...
SPECIALISTS = tuple(
    (spec.name, spec.source, spec.evidence_type) for spec in SPECIALIST_SPECS
)
...
def resolve_specialist_node(
    ...
    """按节点名取规格并绑定共享执行体，未注册的节点回退到 stub。"""
    spec = get_spec(name)
    if spec is None:
        inner = _make_specialist_node(name, source, evidence_type)
    else:
        inner = functools.partial(run_specialist, spec)
