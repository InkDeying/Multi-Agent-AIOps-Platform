...
from app.agents.delegates.registry import SUBAGENTS, SubagentDefinition
from app.harness.core.llm import get_chat_llm
from app.harness.runtime.permissions import PermissionDecision
...
async def _run_subagent(sub: SubagentDefinition, task: str) -> str:
    ...
    流程:
      1. 从 get_base_tools() 拉基础工具, 按 sub.allowed_tools 过滤出 subagent 工具池
    ...
    sub_tools = _resolve_subagent_tools(sub, get_base_tools())
...
# 模块级缓存: delegate tool 定义不变，不需要每次装配目录都重新创建。
_delegate_tools_cache: Optional[List[BaseTool]] = None
...
def get_delegate_tools() -> List[BaseTool]:
    ...
    if _delegate_tools_cache is None:
        _delegate_tools_cache = [_make_delegate_tool(sub) for sub in SUBAGENTS.values()]
        logger.info(
            f"[Subagent] 已注册 {len(_delegate_tools_cache)} 个 delegate 工具: "
            f"{[t.name for t in _delegate_tools_cache]}"
        )
    return list(_delegate_tools_cache)
